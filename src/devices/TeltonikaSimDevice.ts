...
export class TeltonikaSimDevice implements IConfigurable, IReferenceable, IOpenable {
    ...
    private _messages: StateUpdateMessage[] = null;
    ...
    private simulateState() {
        ...
        if (hasCoordinates) {
            ...
            if (RandomFloat.nextFloat(0, 1) > 1) {
                this._logger.info(this._udi, "Adding value to coords");
                this._deltaLatitude = RandomFloat.nextFloat(0, 1) > 0.5 ? 0.001 : -0.001;
                this._deltaLongitude = RandomFloat.nextFloat(0, 1) > 0.5 ? 0.001 : -0.001;
