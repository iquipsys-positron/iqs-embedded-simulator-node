...
export class IndoorDevice implements IConfigurable, IReferenceable, IOpenable {
    ...
    private _useBeaconsInterval: number = 0; // min
    private _useBeaconsEndTime: Date = null;
    private _usingBeacons: boolean = false;
    ...
    private simulateState() {
        ...
        if (this._usingBeacons) {
            ...
            if (hasBeacons) {
                if (RandomBoolean.chance(1, 2))
                    this._beacons = [RandomArray.pick(this._allBeacons)];
                else 
                    this._beacons = this._allBeacons.slice();
            }
        } else {
            this._deltaLatitude = RandomFloat.nextFloat(-0.0001, 0.0001);
            this._deltaLongitude = RandomFloat.nextFloat(-0.0001, 0.0001);

            this._latitude += this._deltaLatitude;
            this._lngtitude += this._deltaLongitude;
            this._speed = RandomInteger.nextInteger(0, 15);
