...
class TwoPointsDevice {
    constructor() {
        ...
        this._latitudeFirstPoint = 0;
        this._lngtitudeFirstPoint = 0;
        this._latitudeSecondPoint = 0;
        this._lngtitudeSecondPoint = 0;
        this._presenceInterval = 0; // min
        this._presenceEndTime = null;
    }
    configure(config) {
        ...
        this._latitude = config.getAsFloatWithDefault('latitude1', this._latitude);
        this._lngtitude = config.getAsFloatWithDefault('longitude1', this._lngtitude);
        this._latitudeFirstPoint = config.getAsFloatWithDefault('latitude1', this._latitudeFirstPoint);
        this._lngtitudeFirstPoint = config.getAsFloatWithDefault('longitude1', this._lngtitudeFirstPoint);
        this._latitudeSecondPoint = config.getAsFloatWithDefault('latitude2', this._latitudeSecondPoint);
        this._lngtitudeSecondPoint = config.getAsFloatWithDefault('longitude2', this._lngtitudeSecondPoint);
        this._presenceInterval = config.getAsIntegerWithDefault('presence', this._presenceInterval);
    ...
    simulateState() {
        if (this._presenceEndTime == null) { // first run
            this._presenceEndTime = new Date();
            this._presenceEndTime.setMinutes(this._presenceEndTime.getMinutes() + this._presenceInterval);
        }
        let currentDate = new Date();
        if (currentDate > this._presenceEndTime) { // change position
            if (this._latitude == this._latitudeFirstPoint && this._lngtitude == this._lngtitudeFirstPoint) {
                this._latitude = this._latitudeSecondPoint;
                this._lngtitude = this._lngtitudeSecondPoint;
            }
            else {
                this._latitude = this._latitudeFirstPoint;
                this._lngtitude = this._lngtitudeFirstPoint;
            }
            this._presenceEndTime.setMinutes(currentDate.getMinutes() + this._presenceInterval);
        }
        ...
        this._power = pip_services3_commons_node_2.RandomBoolean.chance(9, 10);
    }
    toUdi(udi) {
...
exports.TwoPointsDevice = TwoPointsDevice;
//# sourceMappingURL=TwoPointsDevice.js.map