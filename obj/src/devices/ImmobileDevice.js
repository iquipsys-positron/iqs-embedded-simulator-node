...
class ImmobileDevice {
    constructor() {
        ...
        this._immobileEndTime = null;
        this._immobileInterval = 0; // min
        this._moveEndTime = null;
        this._moveInterval = 0; // min
    }
    configure(config) {
        ...
        this._immobileInterval = config.getAsIntegerWithDefault('immobile_interval', this._immobileInterval);
        this._moveInterval = config.getAsIntegerWithDefault('move_interval', this._moveInterval);
    ...
    simulateState() {
        if (this._immobileEndTime == null) { // first run
            this._immobileEndTime = new Date();
            this._immobileEndTime.setMinutes(this._immobileEndTime.getMinutes() + this._immobileInterval);
            this._moveEndTime = new Date();
            this._moveEndTime.setMinutes(this._immobileEndTime.getMinutes() + this._moveInterval);
        }
        let currentDate = new Date();
        if (currentDate <= this._immobileEndTime) { // immobile
        }
        else if (currentDate > this._immobileEndTime) { // start moving
            if (currentDate <= this._moveEndTime) { // move
                this._deltaLatitude = pip_services3_commons_node_3.RandomFloat.nextFloat(-0.001, 0.001);
                this._deltaLongitude = pip_services3_commons_node_3.RandomFloat.nextFloat(-0.001, 0.001);
            ...
            else { // set new time limits
                this._immobileEndTime.setMinutes(this._moveEndTime.getMinutes() + this._immobileInterval);
                this._moveEndTime.setMinutes(this._immobileEndTime.getMinutes() + this._moveInterval);
            }
        }
...
exports.ImmobileDevice = ImmobileDevice;
//# sourceMappingURL=ImmobileDevice.js.map