"use strict";
/**
 * Such devices during _useBeaconsInterval transmitings data with setted beacons
 * and then during same interval as moving device and so on
 */
...
class IndoorDevice {
    constructor() {
        ...
        this._useBeaconsInterval = 0; // min
        this._useBeaconsEndTime = null;
        this._usingBeacons = false;
    }
    configure(config) {
        ...
        this._useBeaconsInterval = config.getAsIntegerWithDefault('beacons_interval', this._useBeaconsInterval);
    ...
    simulateState() {
        if (this._useBeaconsEndTime == null) { // first run
            this._useBeaconsEndTime = new Date();
            this._useBeaconsEndTime.setMinutes(this._useBeaconsEndTime.getMinutes() + this._useBeaconsInterval);
            this._usingBeacons = true;
        }
        let currentDate = new Date();
        if (currentDate > this._useBeaconsEndTime) { // change using beacons
            this._usingBeacons = !this._usingBeacons;
            this._useBeaconsEndTime.setMinutes(currentDate.getMinutes() + this._useBeaconsInterval);
        }
        if (this._usingBeacons) {
            let hasBeacons = this._allBeacons != null && this._allBeacons.length > 0;
            if (hasBeacons) {
                if (pip_services3_commons_node_4.RandomBoolean.chance(1, 2))
                    this._beacons = [RandomArray_1.RandomArray.pick(this._allBeacons)];
                else
                    this._beacons = this._allBeacons.slice();
            }
        }
        else {
            this._deltaLatitude = pip_services3_commons_node_3.RandomFloat.nextFloat(-0.0001, 0.0001);
            this._deltaLongitude = pip_services3_commons_node_3.RandomFloat.nextFloat(-0.0001, 0.0001);
            this._latitude += this._deltaLatitude;
            this._lngtitude += this._deltaLongitude;
            this._speed = pip_services3_commons_node_2.RandomInteger.nextInteger(0, 15);
...
exports.IndoorDevice = IndoorDevice;
//# sourceMappingURL=IndoorDevice.js.map